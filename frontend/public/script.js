...
// ── DEBUG TOGGLE ──────────────────────────────────────────────
document
  .getElementById("debug-toggle-check")
...
async function initEmpresas() {
  try {
    // 🔥 Mostrar sistema INMEDIATAMENTE
    hideLoading();
    showScreen("main");
    ...
    console.log("🔄 Cargando modelos en background...");
    loadModels()
      .then(() => {
      ...
      .then(() => {
        console.log("✅ Perfiles biométricos cargados");
        document.getElementById("stat-usuarios").textContent =
          State.labeledDescriptors.length;
      })
      .catch((err) => {
...
async function getDescriptorsFromImg(img, augment = true) {
  ...
  if (augment) {
    try {
      ...
      const flippedImg = await faceapi.bufferToImage(
        await (await fetch(canvas.toDataURL())).blob(),
      );
      const detFlip = await faceapi
        .detectSingleFace(flippedImg, opts)
        .withFaceLandmarks()
        .withFaceDescriptor();
...
async function loadDescriptors() {
  ...
  try {
    ...
    for (const label of labels) {
      try {
        ...
        const imgRes = await fetch(
          `/get-image?name=${encodeURIComponent(label)}&empresaId=${State.empresaId}`,
        );
        const blob = await imgRes.blob();
        const img = await faceapi.bufferToImage(blob);

        const descriptors = await getDescriptorsFromImg(img, false);
        ...
        if (descriptors.length > 0) {
          ...
          console.log(
            `✅ ${label} — ${descriptors.length} descriptor(es) cargado(s)`,
          );
        } else {
    ...
    console.log(
      `🔥 Total: ${State.labeledDescriptors.length} usuarios con descriptores`,
    );
    ...
    if (State.labeledDescriptors.length > 0) {
      ...
      console.log(
        `✅ FaceMatcher listo (umbral: ${CONFIG.FACE_MATCH_THRESHOLD})`,
      );
    } else {
      console.warn(
        "⚠️ No se cargó ningún descriptor. Verifica que las imágenes tengan rostros claros.",
      );
    }
...
async function startCamera() {
  if (!State.modelsLoaded || !window.faceMatcher) {
    alert("⏳ El sistema aún está cargando IA. Espere unos segundos.");
    return;
  }
  try {
...
function stopCamera() {
  if (State.faceInterval) {
    clearInterval(State.faceInterval);
    State.faceInterval = null;
  }
  if (State.stream) {
...
function onVideoReady() {
  ...
  if (!w || !h) {
    ...
    console.warn(
      `⚠️ Video no listo (intento ${videoReadyRetries}), reintentando...`,
    );
...
function syncCanvasToVideo() {
  ...
  faceapi.matchDimensions(faceCanvas, {
...
async function runFaceDetection() {
  ...
  console.log(
    `🔍 Match: "${result.label}" | Distancia: ${result.distance.toFixed(4)} | Umbral: ${CONFIG.FACE_MATCH_THRESHOLD}`,
  );
  ...
  if (
    ...
    result.distance > CONFIG.FACE_MATCH_THRESHOLD
  ) {
    ...
    setFaceOverlay(
      `✅ ${result.label} (d=${result.distance.toFixed(3)})`,
      "ok",
    );
    State.lastMatchTime = now;
...
function setFaceOverlay(msg, type) {
  ...
  faceOverlay.className =
    type === "ok" ? "face-ok" : type === "warn" ? "face-warn" : "face-err";
...
async function handleSuccessMatch(nombreLabel, distancia) {
  ...
  try {
    const idRes = await fetch(
      `/get-user-id?name=${encodeURIComponent(nombreLabel)}&empresaId=${State.empresaId}`,
    );
    if (idRes.ok) {
      ...
      fijo = data.fijo;
      esAdmin =
        Number(fijo) === 1 || (cargo && cargo.toLowerCase().includes("admin"));
  ...
  if (State.sala === "Mantrab") {
    ...
    const puedeEntrar =
      esAdmin ||
      (State.permisos &&
        State.permisos.salas &&
        State.permisos.salas.includes("Mantrab"));
    ...
    if (esAdmin) {
      setTimeout(() => {
        window.location.href =
          "/dashboard.html?user=" + encodeURIComponent(nombreLabel);
      }, 600);
    } else {
...
async function procesarMantrabPrincipal(usuarioId, nombreLabel) {
  ...
  try {
    ...
    if (data.completo) {
      setFaceOverlay(
        `✅ ${nombreLabel} — Todos identificados (${data.identificados}/${data.personas_detectadas})`,
        "ok",
      );
      ...
      setFaceOverlay(
        `✅ ${nombreLabel} (${data.identificados}/${data.personas_detectadas})`,
        "ok",
      );
      document.getElementById("btnResetMantrab").classList.remove("hidden");
    }
...
async function procesarMantrabSubpuerta(usuarioId, nombreLabel) {
  try {
    ...
    if (data.autorizado) {
      ...
      document
        .getElementById("btnVolverPrincipalFs")
      ...
      setFaceOverlay(
        `✅ ${nombreLabel} (${data.identificados_subpuerta}/${data.personas_detectadas})`,
        "ok",
      );
      setTimeout(() => {
        State.processing = false;
...
function actualizarContadoresMantrab(data) {
  ...
  if (data.completo === true || data.subpuerta_desbloqueada === true) {
    ...
    document.getElementById("mantrab-sub-status").textContent =
      "🔓 Desbloqueada — listos para pasar";
  }
}
...
function resetUIMantrab() {
  ...
  document
    .getElementById("mantrab-sub-card")
    .classList.remove("active", "unlocked");
  document.getElementById("mantrab-sub-card").classList.add("locked");

  document.getElementById("mantrab-principal-status").textContent =
    "🟢 ACTIVA — Identifícate aquí";
  document.getElementById("mantrab-sub-status").textContent =
    "🔒 Bloqueada — pasa primero por la principal";
...
function addLog(nombre, sala, estado, distancia) {
  ...
  document.getElementById("stat-accesos-sub").textContent =
    `${State.logs.length} intentos totales hoy`;
  renderLog();
}
...
function renderLog() {
  ...
  if (State.logs.length === 0) {
    body.innerHTML =
      '<tr><td colspan="5" style="color:var(--muted);text-align:center;padding:1.5rem;">Sin registros aún</td></tr>';
    return;
  }
...
document.getElementById("modal-already-btn").onclick = () =>
  closeModal("modal-already");
...
function renderSalas() {
  ...
  salas.forEach((sala) => {
    ...
    if (State.permisos && !isAdmin) {
      isLocked =
        !Array.isArray(State.permisos.salas) ||
    ...
    btn.onclick = () => {
      ...
      if (sala.esMantrab) {
        alert(
          "🔐 Identifíquese frente a la cámara para acceder al sistema Mantrab",
        );
...
function mostrarPanelMantrab() {
  ...
  if (btnPasar) {
    btnPasar.onclick = () => {
      ...
      document
        .getElementById("mantrab-principal-card-fs")
      ...
      document.getElementById("mantrab-principal-status-fs").textContent =
        "✅ Completada";
      document.getElementById("mantrab-sub-status-fs").textContent =
        "🟢 ACTIVA — Identifícate aquí";
  ...
  if (btnVolverPrincipal) {
    btnVolverPrincipal.onclick = () => {
      ...
      document.getElementById("mantrab-principal-status-fs").textContent =
        "🟢 ACTIVA — Identifíquense aquí";
      document.getElementById("mantrab-sub-status-fs").textContent =
        "🔒 Bloqueada — completen la principal primero";
      ...
      alert(
        "↩ De vuelta a la puerta principal. Las personas que faltaron deben identificarse aquí.",
      );
    };
  }
  ...
  try {
    ...
    videoMantrab.addEventListener(
      ...
      () => {
        ...
        intervalMantrab = setInterval(
          () => runFaceDetectionMantrab(),
          CONFIG.DETECTION_INTERVAL,
        );
        console.log(
          `🎥 Cámara Mantrab activa: ${videoMantrab.videoWidth}x${videoMantrab.videoHeight}`,
        );
      },
      { once: true },
    );
...
function stopCameraMantrab() {
  if (intervalMantrab) {
    clearInterval(intervalMantrab);
    intervalMantrab = null;
  }
  if (streamMantrab) {
    streamMantrab.getTracks().forEach((t) => t.stop());
    streamMantrab = null;
  }
  const videoMantrab = document.getElementById("videoMantrab");
...
async function runFaceDetectionMantrab() {
  ...
  const detections = await faceapi
    .detectAllFaces(videoMantrab, opts)
    .withFaceLandmarks()
    .withFaceDescriptors();
  ...
  const result = window.faceMatcher.findBestMatch(bestDetection.descriptor);
  console.log(
    `🔍 [MANTRAB] Match: "${result.label}" | Distancia: ${result.distance.toFixed(4)}`,
  );
  ...
  try {
    const idRes = await fetch(
      `/get-user-id?name=${encodeURIComponent(result.label)}&empresaId=${State.empresaId}`,
    );
    if (idRes.ok) {
      const data = await idRes.json();
...
function resetUIMantrabFullscreen() {
  ...
  document
    .getElementById("mantrab-sub-card-fs")
    .classList.remove("active", "unlocked");
