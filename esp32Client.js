...
const ESP32_BASE_URL = process.env.ESP32_URL || 'http://192.168.0.26';

class ESP32Client {

    static async abrirPuerta(origen = 'general') {
        try {
            const response = await axios.post(`${ESP32_BASE_URL}/abrir`, {
                origen,
                angulo: 90
            }, {
                timeout: 3000
    ...
    static async cerrarPuerta(origen = 'general') {
        try {
            const response = await axios.post(`${ESP32_BASE_URL}/cerrar`, {
                origen,
                angulo: 0
            }, {
                timeout: 3000
