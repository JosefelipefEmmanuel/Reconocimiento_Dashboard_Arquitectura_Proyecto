...
const ESP32Client = require("./esp32Client");
...
app.post("/upload", upload.single("photo"), (req, res) => {
  ...
  if (!req.file) return res.status(400).send("Sin imagen");
  ...
  db.query(
    ...
    (err, result) => {
      ...
      if (result[0].count > 0) return res.status(400).send("Usuario ya existe");
...
app.post("/register-entry", (req, res) => {
  ...
  db.query(
    ...
    (err, results) => {
      ...
      const esAdmin = Number(user.fijo) === 1 || cargo.includes("admin");
...
app.post("/mantrap/principal", (req, res) => {
  ...
  if (!sesionMantrab.activa) {
    ...
    console.log(
      `🆕 Nueva sesión Mantrab — ${personas_detectadas} personas detectadas`,
    );
  } else {
  ...
  console.log(
    `🚪 Mantrab/principal — ${identificados}/${total} ${completo ? "✅" : "⏳"}`,
  );
...
// ────────────────────────────────────────────────
app.post("/mantrap/subpuerta", async (req, res) => {
  ...
  if (!sesionMantrab.activa) {
    return res.status(403).json({
      ...
      codigo: "NO_SESSION",
      motivo:
        "⛔ No hay sesión activa. Identifíquese primero en la puerta principal.",
    });
  }
  ...
  if (!sesionMantrab.identificados_principal.includes(usuario_id)) {
    return res.status(403).json({
      ...
      motivo:
        "⛔ ACCESO DENEGADO. Usted no se identificó en la puerta principal.",
    });
  }
  ...
  if (autorizado) {
    ...
    const respuestaESP32 = await ESP32Client.abrirPuerta("mantrap-subpuerta");
...
app.get("/mantrap/estado", (req, res) => {
  res.json({
    ...
    principal_completa:
      ...
      sesionMantrab.identificados_principal.length >=
        sesionMantrab.personas_detectadas,
  });
});

// ============================
// DASHBOARD ADMIN - DATOS BD
// ============================

app.get("/api/dashboard/resumen", (req, res) => {
  const data = {};

  db.query("SELECT COUNT(*) AS total FROM tabla_usuarios", (err, usuarios) => {
    if (err) return res.status(500).json({ error: "Error usuarios" });

    data.usuarios = usuarios[0].total;

    db.query(
      "SELECT COUNT(*) AS total FROM registro WHERE DATE(hora_entrada) = CURDATE()",
      (err, accesos) => {
        if (err) return res.status(500).json({ error: "Error accesos" });

        data.accesosHoy = accesos[0].total;

        db.query(
          "SELECT COUNT(*) AS total FROM intentos_fallidos",
          (err, fallidos) => {
            if (err) return res.status(500).json({ error: "Error fallidos" });

            data.intentosFallidos = fallidos[0].total;
            data.salas = SALAS_VALIDAS.length;
...
app.get("/api/dashboard/accesos", (req, res) => {
  db.query(
    `SELECT 
        ...
        u.cargo,
        r.ubicacion,
        r.resultado_autenticacion,
        r.hora_entrada
     FROM registro r
     LEFT JOIN tabla_usuarios u ON u.id = r.usuario_id
...
app.get("/api/dashboard/fallidos", (req, res) => {
  db.query(
    `SELECT 
        id,
        cedula,
...
app.get("/api/dashboard/empresas", (req, res) => {
  db.query(
    "SELECT id, nombre FROM empresas ORDER BY nombre ASC",
    (err, rows) => {
      if (err) return res.status(500).json([]);
...
app.get("/api/dashboard/salas", (req, res) => {
  res.json(SALAS_VALIDAS);
});

// ============================
// PRUEBAS ESP32
// ============================
app.post("/test/esp32/abrir", async (req, res) => {
  const respuesta = await ESP32Client.abrirPuerta("prueba-manual");
  res.json(respuesta);
});

app.post("/test/esp32/cerrar", async (req, res) => {
  const respuesta = await ESP32Client.cerrarPuerta("prueba-manual");
  res.json(respuesta);
});

app.get("/test/esp32/sensores", async (req, res) => {
  const respuesta = await ESP32Client.obtenerSensores();
  res.json(respuesta);
});
// ============================
// KEEP ALIVE DB
